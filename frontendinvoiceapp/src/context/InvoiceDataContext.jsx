...
export const InvoiceDataProvider = ({ children }) => {
    ...
    const getOrCreateIdentifier = () => {
        let identifier = Cookies.get('identifier');
        if (!identifier) {
            identifier = uuidv4();
            Cookies.set('identifier', identifier, { expires: 30 }); // Expire dans 30 jours
        }
        return identifier;
    };
    ...
    const handleDownloadInvoice = async () => {
        ...
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/facture/create`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': user ? `Bearer ${user.token}` : '', // Si l'utilisateur est connecté
                },
                body: JSON.stringify({ ...invoiceData, identifier }), // Inclure l'identifiant dans le corps de la requête
            });
    
            // Vérification de la réponse de l'API
            if (!response.ok) {
                if (response.status === 403) {
                    setShowErrorMessage('Vous avez atteint la limite de 3 factures pour les utilisateurs non abonnés.');
                } else {
                    setShowErrorMessage('Une erreur est survenue lors de la création de la facture.');
                }
                return; // Sortie de la fonction si l'API renvoie une erreur
            }
    
            // L'appel à l'API a réussi, continuer avec la génération du PDF
